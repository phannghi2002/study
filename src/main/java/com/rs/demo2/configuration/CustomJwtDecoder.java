...
public class CustomJwtDecoder implements JwtDecoder {
	...
	public Jwt decode(String token) throws JwtException {
		try {
			// neu dung ham nay nay se loi: authenticationService.introspect(token), vi ta can truyen IntrospectRequest
			// vao chu khong phai truyen
			// token vao, nen ta dung builder de tao va gan gia tri

			// neu khong dung builder thi ta co the dung nhu sau:  IntrospectRequest introspectRequest = new
			// IntrospectRequest();
			...
			if (!response.isValid()) throw new BadJwtException("Token is valid");
